...
void display_results(const struct Video *videos, int video_count, int offset) {
    ...
    printf("D: download video, Q: toggle quality, R: reload search, S: new search, 0: exit\n");
}
...
int main(void) {
    ...
    while (running) {
        ...
        if (c >= '1' && c <= '9') {
            ...
            // Prompt for a number input to select a video to download.
            printf("\nEnter video number to download (1-%d): ", MAX_RESULTS);
            char choice_char;
            if (read(STDIN_FILENO, &choice_char, 1) != 1)
                continue;
            int choice = choice_char - '0';
            if (choice >= 1 && choice <= MAX_RESULTS && (offset + choice - 1) < video_count) {
                printf("\nDownloading video...\n");
